...
let { addSongToDb, getSongById, getMusicUrl, getRandomSong } = require('../dao/songDao');
...
const getDoubanMovie = movieName => {
    ...
    let url = `https://api.douban.com/v2/movie/search?` + querystring.stringify({ count: 4, q: movieName.trim() })
    console.log(url, '----url')
    return promisify(request)({ url, forever: true }).then(info => {
    ...
    return getVideos(videoName).then(videos => {
        ...
        return Promise.resolve(videos.map(vd => {
            ...
            return {
                ...
                title: title || vd.id,
                picurl: vd.thumbnailUrl || 'http://yongwang.space:7000/Videos/video_thumbnails/' + vd.screenshot,
                // url: 'http://yongwang.space:7000/Videos/insta' + vd.id +'.mp4'
                url: `http://yongwang.space:7000/Videos/${vd.relativePath}`
            }
        }))
    }).catch(console.log)
